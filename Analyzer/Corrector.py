...
def Command_Exploration(message_Result, payload_message):
    ...
    for template_field in template_format:
        ...
        logging.info(f"alignmentInCluster_avg:{alignmentInCluster_avg}")
        logging.info(f"alignmentBetweenClusters_avg:{alignmentBetweenClusters_avg}")


        logging.info(f"----- template_field {k}")
        if max_score < alignmentInCluster_avg:
            if template_field not in candidates or config.notConformCommand(message_Result,template_field,template_format):
                if template_field not in candidates:
                    logging.info(f"template_field {k} is not in candidates")
                if config.notConformCommand(message_Result,template_field,template_format):
                    logging.info(f"template_field {k} does not conform with the command")
                continue
    ...
    for k,v in alignmentInCluster.items():
        logging.info(f"template_field:{k}, alignmentInCluster:{alignmentInCluster[k]}, alignmentBetweenCluster:{alignmentBetweenClusters[k]}")
...
def Type_Validation(Command_field, Command_size,message_Result, v):
    ...
    for index in v:
        ...
        logging.info(f"fields_entropies:{fields_entropies}")
        if fields_entropies == []:
            median_value = 0
        else:
        ...
        for (f, entro) in fields_entropies:
            if f == Command_field:
                continue
            logging.info(f"f : {f}, entro : {entro}")
...
def Validation(message_Result,payload_message):
    ...
    logging.info(f"Command_field:{Command_field}")
